...
n_episodes = 100000       # Number of episodes to train on
...
for episode in tqdm(range(n_episodes)):
    ...
    # 1. PRINT EPSILON EVERY 250 EPISODES
    if (episode + 1) % 250 == 0:
        # We calculate the average of the last 100 just for the log display
        recent_avg = np.mean(reward_list[-100:]) if len(reward_list) >= 100 else np.mean(reward_list)
        # Using tqdm.write prevents the progress bar from breaking visually
        tqdm.write(f"Episode {episode+1} | Epsilon: {agent.epsilon:.4f} | Avg Reward (last 100): {recent_avg:.1f}")

    # 2. CHECK STOPPING CONDITION (Avg >= 475 over last 100 episodes)
    if len(reward_list) >= SOLVED_WINDOW:
        # Calculate average of the last 100 rewards
        avg_reward = np.mean(reward_list[-SOLVED_WINDOW:])
        
        if avg_reward >= SOLVED_AVG_REWARD:
